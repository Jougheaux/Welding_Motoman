...
slice_height = 0.5 #mm per sliced layer
...
num_layers = 60
num_baselayers = 0
height_offset = -5 #mm this accounts for where the camera thinks the bead is
layer_height = .5 # mm
bead_width = 5 # mm
ONLINE = True 
ARCON = True

height_adjustment2 = 13
...
true_height_offset = height_offset-slicing_meta['z_offset'] ##add this -15 to the yaml file and read it here
slice_height = slicing_meta['slice_height'] #mm per sliced layer
...
for layer in range(0, num_layers+1):
    ...
    if layer >0:
        ...
        for seg in range(num_segs):      
            ...
            seg_heights.append(sum(heights_prev)/len(heights_prev))
        average_layer_height = sum(seg_heights)/len(seg_heights)+true_height_offset+height_adjustment2
        layer_heights.append(round(convertmm*average_layer_height/slice_height))
        
    
        print("Layer Position:"+str(layer_heights[layer]*slice_height))
            """
        
        layer_heights.append(0) #this shouldnt be here just a fix for no camera
        overrideLayerHeight = input("If layer is wrong input desired ideal layer position")
        if overrideLayerHeight == "":
            print("cool")
        else:
            layer_heights[-1] = round(float(overrideLayerHeight)/slice_height)
