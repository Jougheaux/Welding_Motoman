...
for layer in range(num_baselayers):
    num_segs = len(glob.glob(f'{data_dir}curve_sliced_relative/baselayer{layer}_*.csv'))
    print("Number of Segments: ", num_segs)
    for seg in range(num_segs):
        ### Load Data
        curve_sliced_js = np.loadtxt(
            data_dir + f"curve_sliced_js/MA2010_base_js{layer}_{seg}.csv", delimiter=","
        ).reshape((-1, 6))

        positioner_js = np.loadtxt(
            data_dir + f"curve_sliced_js/D500B_base_js{layer}_{seg}.csv", delimiter=","
        )
        curve_sliced_relative = np.loadtxt(
            data_dir + f"curve_sliced_relative/baselayer{layer}_{seg}.csv", delimiter=","
        )
        ...
        print(int(base_feedrate_cmd / 10) + job_offset)
...
for layer in range(0, num_layers):
    num_segs = len(glob.glob(f'{data_dir}curve_sliced_relative/slice{layer}_*.csv'))
    print("Number of Segments: ", num_segs)
    for seg in range(num_segs):
        ### Load Data
        curve_sliced_js = np.loadtxt(
            data_dir + f"curve_sliced_js/MA2010_js{layer}_{seg}.csv", delimiter=","
        ).reshape((-1, 6))

        positioner_js = np.loadtxt(
            data_dir + f"curve_sliced_js/D500B_js{layer}_{seg}.csv", delimiter=","
        )
        curve_sliced_relative = np.loadtxt(
            data_dir + f"curve_sliced_relative/slice{layer}_{seg}.csv", delimiter=","
        )
