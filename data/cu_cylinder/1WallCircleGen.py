...
y_offset = 0
z_offset = 50
layer_angular_shift = 20*np.pi/180
...
for layer in range(num_layers):
	curve_dense[layer*points_per_layer:(layer+1)*points_per_layer,0]=wall_radius*np.cos(t+(layer+num_base)*layer_angular_shift)+x_offset
	curve_dense[layer*points_per_layer:(layer+1)*points_per_layer,1]=wall_radius*np.sin(t+(layer+num_base)*layer_angular_shift)+y_offset
...
while (r>0):
    ...
    if(r<0):
        ...
        for layer in range(num_base):
        	curve_base[layer*points_per_base:(layer+1)*points_per_base,0]=r*np.cos(t+layer_angular_shift)+x_offset
        	curve_base[layer*points_per_base:(layer+1)*points_per_base,1]=r*np.sin(t+layer_angular_shift)+y_offset
